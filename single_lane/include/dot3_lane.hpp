...
template <int PE_PER_LANE, bool ENABLE_ACCUM = false, int ACC_WIDTH = 32>
struct Dot3LaneWidths {
  static_assert(PE_PER_LANE > 0, "PE_PER_LANE must be positive");
  static constexpr int kPeOutputWidth = 24;
  static constexpr int kLaneSumWidth =
      kPeOutputWidth + ceil_log2_constexpr(PE_PER_LANE);
  static_assert(!ENABLE_ACCUM || ACC_WIDTH >= kLaneSumWidth,
                "ACC_WIDTH must hold one complete lane sum");
  static constexpr int kOutputWidth =
      ENABLE_ACCUM ? ACC_WIDTH : kLaneSumWidth;
};
...
template <int PE_PER_LANE, bool ENABLE_ACCUM = false, int ACC_WIDTH = 32>
void dot3_lane(
    const ap_int<8> activation[PE_PER_LANE][3],
    const ap_int<8> weight[PE_PER_LANE][3], bool acc_clear, bool acc_valid,
    bool acc_last,
    ap_int<Dot3LaneWidths<PE_PER_LANE, ENABLE_ACCUM, ACC_WIDTH>::kOutputWidth>&
        result,
    bool& result_valid) {
...
#pragma HLS allocation function instances=dsp58_dot3_int8 limit=PE_PER_LANE

  using Widths = Dot3LaneWidths<PE_PER_LANE, ENABLE_ACCUM, ACC_WIDTH>;
  ap_int<24> partial[PE_PER_LANE];
...
#pragma HLS unroll
  ...
  if constexpr (ENABLE_ACCUM) {
    static ap_int<ACC_WIDTH> accumulator = 0;
#pragma HLS bind_op variable=accumulator op=add impl=fabric
    result_valid = acc_valid && acc_last;
    if (acc_valid) {
      const ap_int<ACC_WIDTH> base = acc_clear ? ap_int<ACC_WIDTH>(0)
                                               : accumulator;
      const ap_int<ACC_WIDTH> next = base + ap_int<ACC_WIDTH>(lane_sum);
