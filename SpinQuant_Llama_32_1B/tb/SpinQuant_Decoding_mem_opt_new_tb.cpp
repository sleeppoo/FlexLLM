...
#include "SpinQuant_Decoding_mem_opt_new.h"

#include <algorithm>
#include <numeric>
#include <cmath>

template <int block_parallel, int weight_parallel, int input_dim, int output_dim, int unroll_num = 2>
void decoding_read_int4_bin_as_int8_weight_mmap(
    const std::string& layer_name,
    int layer,
    std::vector<hls::vector<ap_int<8>, weight_parallel>, tapa::aligned_allocator<hls::vector<ap_int<8>, weight_parallel>>> weight_mmaps[block_parallel/unroll_num],
    int mmap_offset,
    int unroll_id_0 = 0,
    int unroll_id_1 = 1,
    ...
    for (int block_id = 0; block_id < block_parallel/unroll_num; ++block_id) {
        int actual_block_id_0 = unroll_id_0 * (block_parallel/unroll_num) + block_id;
        int actual_block_id_1 = unroll_id_1 * (block_parallel/unroll_num) + block_id;
        for (int n = 0; n < output_dim/block_parallel; ++n) {
            // read one whole row (input_dim bytes) for each of the two output channels
            std::vector<int8_t> data_0(input_dim);
            std::vector<int8_t> data_1(input_dim);

            const std::streamoff off0 = static_cast<std::streamoff>( actual_block_id_0 * (output_dim/block_parallel) + n) * input_dim * sizeof(int8_t);
            const std::streamoff off1 = static_cast<std::streamoff>( actual_block_id_1 * (output_dim/block_parallel) + n) * input_dim * sizeof(int8_t);

            fin.seekg(off0, std::ios::beg);
            fin.read(reinterpret_cast<char*>(data_0.data()), data_0.size() * sizeof(int8_t));
            if (!fin) { std::cerr << "Read error @ row " << actual_block_id_0 * (output_dim/block_parallel) + n << " in " << name << "\n"; return; }

            fin.seekg(off1, std::ios::beg);
            fin.read(reinterpret_cast<char*>(data_1.data()), data_1.size() * sizeof(int8_t));
            if (!fin) { std::cerr << "Read error @ row " << actual_block_id_1 * (output_dim/block_parallel) + n << " in " << name << "\n"; return; }
...
void SpinQuant_Decoding_test(int argc, char* argv[]) {
    ...
    vector<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL>, tapa::aligned_allocator<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL>>> w_qkvo_FFN_mmaps_quart_01_k_caches[T_QKVO_FFN_BLOCK_PARALLEL/4];
    for (int i = 0; i < T_QKVO_FFN_BLOCK_PARALLEL/4; i++) {
        w_qkvo_FFN_mmaps_quart_01_k_caches[i].resize(w_qkvo_FFN_size + k_caches_size);
    }
    vector<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL>, tapa::aligned_allocator<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL>>> w_qkvo_FFN_mmaps_quart_23_v_caches[T_QKVO_FFN_BLOCK_PARALLEL/4];
    for (int i = 0; i < T_QKVO_FFN_BLOCK_PARALLEL/4; i++) {
        w_qkvo_FFN_mmaps_quart_23_v_caches[i].resize(w_qkvo_FFN_size + v_caches_size);
    }
    ...
    //     initInt8Vec(w_qkvo_FFN_mmaps_quart_0_k_caches[i], DEC_QKVO_FFN_W_PARALLEL/2);
    // }
    // for (int i = 0; i < T_QKVO_FFN_BLOCK_PARALLEL/2; i++) {
    //     initInt8Vec(w_qkvo_FFN_mmaps_quart_1_v_caches[i], DEC_QKVO_FFN_W_PARALLEL/2);
    // }


    for (int i = 0; i < DECODER_LAYER_NUM; i++) {
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, KV_HIDDEN_DIM, 4>(
            "k_proj", i, w_qkvo_FFN_mmaps_quart_01_k_caches, w_kv_addr_bias + i * KV_HIDDEN_DIM_PAD / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 0, 1
        );
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, KV_HIDDEN_DIM, 4>(
            "k_proj", i, w_qkvo_FFN_mmaps_quart_23_v_caches, w_kv_addr_bias + i * KV_HIDDEN_DIM_PAD / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 2, 3
        );
    }
    ...
    for (int i = 0; i < DECODER_LAYER_NUM; i++) {
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, KV_HIDDEN_DIM, 4>(
            "v_proj", i, w_qkvo_FFN_mmaps_quart_01_k_caches, w_kv_addr_bias + i * KV_HIDDEN_DIM_PAD / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 0, 1, KV_HIDDEN_DIM/T_QKVO_FFN_BLOCK_PARALLEL
        );
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, KV_HIDDEN_DIM, 4>(
            "v_proj", i, w_qkvo_FFN_mmaps_quart_23_v_caches, w_kv_addr_bias + i * KV_HIDDEN_DIM_PAD / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 2, 3, KV_HIDDEN_DIM/T_QKVO_FFN_BLOCK_PARALLEL
        );
    }
    cout << "Decoding: Finished reading v_proj weights." << endl;

    for (int i = 0; i < DECODER_LAYER_NUM; i++) {
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, HIDDEN_DIM, 4>(
            "q_proj", i, w_qkvo_FFN_mmaps_quart_01_k_caches, w_q_addr_bias + i * HIDDEN_DIM / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 0, 1
        );
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, HIDDEN_DIM, 4>(
            "q_proj", i, w_qkvo_FFN_mmaps_quart_23_v_caches, w_q_addr_bias + i * HIDDEN_DIM / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 2, 3
        );
    }
    ...
    for (int i = 0; i < DECODER_LAYER_NUM; i++) {
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, HIDDEN_DIM, 4>(
            "o_proj", i, w_qkvo_FFN_mmaps_quart_01_k_caches, w_o_addr_bias + i * HIDDEN_DIM / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 0, 1
        );
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, HIDDEN_DIM, 4>(
            "o_proj", i, w_qkvo_FFN_mmaps_quart_23_v_caches, w_o_addr_bias + i * HIDDEN_DIM / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 2, 3
        );
    }
    cout << "Decoding: Finished reading o_proj weights." << endl;

    for (int i = 0; i < DECODER_LAYER_NUM; i++) {
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, INTER_DIM, 4>(
            "up_proj", i, w_qkvo_FFN_mmaps_quart_01_k_caches, w_ffn_up_addr_bias + i * INTER_DIM / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 0, 1
        );
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, INTER_DIM, 4>(
            "up_proj", i, w_qkvo_FFN_mmaps_quart_23_v_caches, w_ffn_up_addr_bias + i * INTER_DIM / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 2, 3
        );
    }
    ...
    for (int i = 0; i < DECODER_LAYER_NUM; i++) {
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, INTER_DIM, 4>(
            "gate_proj", i, w_qkvo_FFN_mmaps_quart_01_k_caches, w_ffn_gate_addr_bias + i * INTER_DIM / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 0, 1
        );
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, INTER_DIM, 4>(
            "gate_proj", i, w_qkvo_FFN_mmaps_quart_23_v_caches, w_ffn_gate_addr_bias + i * INTER_DIM / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * HIDDEN_DIM, 2, 3
        );
    }
    cout << "Decoding: Finished reading gate_proj weights." << endl;

    for (int i = 0; i < DECODER_LAYER_NUM; i++) {
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, HIDDEN_DIM, 4>(
            "down_proj", i, w_qkvo_FFN_mmaps_quart_01_k_caches, w_ffn_down_addr_bias + i * HIDDEN_DIM / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * INTER_DIM, 0, 1
        );
        decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, HIDDEN_DIM, 4>(
            "down_proj", i, w_qkvo_FFN_mmaps_quart_23_v_caches, w_ffn_down_addr_bias + i * HIDDEN_DIM / (T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL) * INTER_DIM, 2, 3
        );
    }
    cout << "Decoding: Finished reading down_proj weights." << endl;

    decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, VOCAB_SIZE_PAD, 4>(
        "lm_head", 0, w_qkvo_FFN_mmaps_quart_01_k_caches, w_vocab_addr_bias, 0, 1
    );
    decoding_read_int4_bin_as_int8_weight_mmap<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, HIDDEN_DIM, VOCAB_SIZE_PAD, 4>(
        "lm_head", 0, w_qkvo_FFN_mmaps_quart_23_v_caches, w_vocab_addr_bias, 2, 3
    );
    ...
    // Apply initialization
                ...
                for (int h = 0; h < KV_HEAD_NUM; h++) {
                    ...
                    for (int j = 0; j < HEAD_DIM; j++) {
                        ...
                        if(i < MAX_PRE_SEQ_LEN){
                            ...
                            w_qkvo_FFN_mmaps_quart_01_k_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][w_qkvo_FFN_size + k_idx][k_sub_idx] = ap_int<8>(qval_k);
                        }
                        else {
                            // k_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][idx][sub_idx] = ap_int<8>(0);
                            w_qkvo_FFN_mmaps_quart_01_k_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][w_qkvo_FFN_size + k_idx][k_sub_idx] = ap_int<8>(0);
                        }
                        ...
                        if(i < MAX_PRE_SEQ_LEN){
                            ...
                            w_qkvo_FFN_mmaps_quart_23_v_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][w_qkvo_FFN_size + v_idx][v_sub_idx] = ap_int<8>(qval_v);
                        }
                        else {
                            // v_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][idx][sub_idx] = ap_int<8>(0);
                            w_qkvo_FFN_mmaps_quart_23_v_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][w_qkvo_FFN_size + v_idx][v_sub_idx] = ap_int<8>(0);
                        }
                    }
                }
            }
        ...
        int64_t kernel_time_ns = tapa::invoke(
            ...
            tapa::read_write_mmaps<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL>, T_QKVO_FFN_BLOCK_PARALLEL/4>(w_qkvo_FFN_mmaps_quart_01_k_caches),
            tapa::read_write_mmaps<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL>, T_QKVO_FFN_BLOCK_PARALLEL/4>(w_qkvo_FFN_mmaps_quart_23_v_caches),
