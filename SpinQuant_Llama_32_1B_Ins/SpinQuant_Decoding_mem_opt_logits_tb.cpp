...
void SpinQuant_Decoding_test(int argc, char* argv[]) {
    ...
    // decoder token idx
    vector<sampling_idx_logits_pair, tapa::aligned_allocator<sampling_idx_logits_pair>> sampled_token_idx_logits_mmap(MAX_DEC_SEQ_LEN * (LOGITS_MAX_K + 1));
    ...
    // Apply initialization
        ...
        int test_prefill_len = 32 + 4 * run;
        int test_decode_len = 4;
        ...
        const int first_token_idx = 128000 % VOCAB_SIZE;
        for (size_t idx = 0; idx < io_init_vecs; ++idx) {
            io_mmap[idx] = vocab_lib[first_token_idx * io_init_vecs + idx];
        }
        ...
        int64_t kernel_time_ns = tapa::invoke(
            ...
            tapa::write_only_mmap<sampling_idx_logits_pair>(sampled_token_idx_logits_mmap),
            // MAX_PRE_SEQ_LEN,
            test_prefill_len,
            // MAX_DEC_SEQ_LEN
            test_decode_len
        ...
        if (!fout) {
            ...
            for (int i = 0; i < test_decode_len; ++i) {
                fout << sampled_token_idx_logits_mmap[i*(LOGITS_MAX_K + 1) + LOGITS_MAX_K].idx;
                if (i + 1 < test_decode_len) fout << " ";
            }
            fout << "\n";
        }
    }
