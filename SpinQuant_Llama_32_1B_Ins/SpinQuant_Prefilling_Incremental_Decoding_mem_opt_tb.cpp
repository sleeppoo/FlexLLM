...
DEFINE_int32(num_requests, 1, "number of prompt requests to run");
DEFINE_bool(wait_each_request, false, "wait for a character before each request, matching the interactive demo style");
DEFINE_int32(incremental_rounds, 4, "number of short decode invocations after one prefill");
DEFINE_int32(gamma, 4, "number of draft tokens to request from each short decode invocation");
DEFINE_int32(accepted_per_round, -1, "simulated accepted draft tokens per round; -1 means accept all gamma tokens");
DEFINE_int32(replacement_token_id, 128000, "target replacement token used when accepted_per_round is smaller than gamma");
...
int SpinQuant_Prefilling_Decoding_test(int argc, char* argv[]) {
    ...
    if (FLAGS_num_requests <= 0) {
        std::cerr << "--num_requests must be positive\n";
        return 1;
    }
    if (FLAGS_incremental_rounds <= 0) {
        std::cerr << "--incremental_rounds must be positive\n";
        return 1;
    }
    if (FLAGS_gamma <= 0 || FLAGS_gamma > MAX_DEC_SEQ_LEN) {
        std::cerr << "--gamma must be in 1.." << MAX_DEC_SEQ_LEN << "\n";
        return 1;
    }
    ...
    vector<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL/2>, tapa::aligned_allocator<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL/2>>> canonical_k_caches[T_QKVO_FFN_BLOCK_PARALLEL/2];
    for (int i = 0; i < T_QKVO_FFN_BLOCK_PARALLEL/2; i++) {
        canonical_k_caches[i].resize(k_caches_size);
    }
    vector<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL/2>, tapa::aligned_allocator<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL/2>>> canonical_v_caches[T_QKVO_FFN_BLOCK_PARALLEL/2];
    for (int i = 0; i < T_QKVO_FFN_BLOCK_PARALLEL/2; i++) {
        canonical_v_caches[i].resize(v_caches_size);
    }
    ...
    const int num_runs = FLAGS_num_requests;
    ...
    // Apply initialization
        ...
        auto zero_canonical_cache = [&]() {
            for (int i = 0; i < DEC_HEAD_PARALLEL; i++){
                for(auto &vec : canonical_k_caches[i]){
                    for(int k = 0; k < DEC_K_PARALLEL; k++) vec[k] = 0;
                }
                for(auto &vec : canonical_v_caches[i]){
                    for(int k = 0; k < DEC_V_PARALLEL; k++) vec[k] = 0;
                }
            }
        };

        auto copy_prefill_to_canonical = [&](int pre_seq_len) {
                ...
                for (int i = 0; i < pre_seq_len; i++) {
                    for (int h = 0; h < KV_HEAD_NUM; h++) {
                        for (int j = 0; j < HEAD_DIM; j++) {
                            ...
                            canonical_k_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][write_idx][write_sub_idx] = pref_k_cache[read_idx][read_sub_idx];
                            ...
                            canonical_v_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][write_idx][write_sub_idx] = pref_v_cache[read_idx][read_sub_idx];
                        }
                    }
                }
            }
        ...
        auto restore_canonical_prefix_to_device = [&](int seq_len) {
            zero_device_decode_cache();
            for(int layer = 0; layer < DECODER_LAYER_NUM; layer++){
                    ...
                    for (int h = 0; h < KV_HEAD_NUM; h++) {
                        for (int j = 0; j < HEAD_DIM; j++) {
                            int write_idx = ((layer * KV_HEAD_NUM/DEC_HEAD_PARALLEL + h % (KV_HEAD_NUM/DEC_HEAD_PARALLEL)) * MAX_SUM_SEQ_LEN + i)/DEC_K_PARALLEL *
                                        HEAD_DIM + j;
                            int write_sub_idx = i % DEC_K_PARALLEL;
                            int bank = h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL);
                            w_qkvo_FFN_mmaps_half_0_k_caches[bank][w_qkvo_FFN_size + write_idx][write_sub_idx] =
                                canonical_k_caches[bank][write_idx][write_sub_idx];

                            write_idx = ((layer * (KV_HEAD_NUM/DEC_HEAD_PARALLEL) + h % (KV_HEAD_NUM/DEC_HEAD_PARALLEL)) * HEAD_DIM + j)/DEC_V_PARALLEL * MAX_SUM_SEQ_LEN + i;
                            write_sub_idx = j % DEC_V_PARALLEL;
                            w_qkvo_FFN_mmaps_half_1_v_caches[bank][w_qkvo_FFN_size + write_idx][write_sub_idx] =
                                canonical_v_caches[bank][write_idx][write_sub_idx];
                        }
                    }
                }
            }
        ...
        auto commit_device_position_to_canonical = [&](int seq_id) {
            for(int layer = 0; layer < DECODER_LAYER_NUM; layer++){
                for (int h = 0; h < KV_HEAD_NUM; h++) {
                    for (int j = 0; j < HEAD_DIM; j++) {
                        int write_idx = ((layer * KV_HEAD_NUM/DEC_HEAD_PARALLEL + h % (KV_HEAD_NUM/DEC_HEAD_PARALLEL)) * MAX_SUM_SEQ_LEN + seq_id)/DEC_K_PARALLEL *
                                    HEAD_DIM + j;
                        int write_sub_idx = seq_id % DEC_K_PARALLEL;
                        int bank = h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL);
                        canonical_k_caches[bank][write_idx][write_sub_idx] =
                            w_qkvo_FFN_mmaps_half_0_k_caches[bank][w_qkvo_FFN_size + write_idx][write_sub_idx];

                        write_idx = ((layer * (KV_HEAD_NUM/DEC_HEAD_PARALLEL) + h % (KV_HEAD_NUM/DEC_HEAD_PARALLEL)) * HEAD_DIM + j)/DEC_V_PARALLEL * MAX_SUM_SEQ_LEN + seq_id;
                        write_sub_idx = j % DEC_V_PARALLEL;
                        canonical_v_caches[bank][write_idx][write_sub_idx] =
                            w_qkvo_FFN_mmaps_half_1_v_caches[bank][w_qkvo_FFN_size + write_idx][write_sub_idx];
                    }
        ...
        auto fill_demo_rand_seeds = [&]() {
            for (int i = 0; i < MAX_DEC_SEQ_LEN; i++) {
                ...
                float x = std::fabs(z0) / 3.0f;
                if (x > 1.0f) x = 0.0f;
                rand_seeds_mmap[i] = x;
            }
        };

        zero_canonical_cache();
        copy_prefill_to_canonical(test_pre_seq_len);
        cout << "Decoding: Finished loading prefill KV into canonical host cache." << endl;

        int seq_len = test_pre_seq_len;
        int last_token_id = token_idx[token_idx.size() - 1];
        int64_t request_dec_total_time_ns = 0;
        std::vector<llama_token> ids_for_decode;

        for (int round = 0; round < FLAGS_incremental_rounds; round++) {
            if (seq_len + FLAGS_gamma > MAX_SUM_SEQ_LEN) {
                std::cerr << "Incremental decode would exceed MAX_SUM_SEQ_LEN at round " << round << "\n";
                break;
            }

            fill_demo_rand_seeds();
            for (int idx = 0; idx < dec_io_mmap.size(); idx++) {
                dec_io_mmap[idx] = 0.0f;
            }
            for (int idx = 0; idx < sampled_token_idx_mmap.size(); idx++) {
                sampled_token_idx_mmap[idx] = 0;
            }
            for (size_t idx = 0; idx < HIDDEN_DIM/T_BLOCK_PARALLEL; ++idx) {
                dec_io_mmap[idx] = vocab_lib[last_token_id * (HIDDEN_DIM/T_BLOCK_PARALLEL) + idx];
            }
            restore_canonical_prefix_to_device(seq_len);

            cout << "Incremental decode round " << round
                 << " begins: pre_seq_len=" << seq_len
            ...
            request_dec_total_time_ns += dec_kernel_time_ns;
            dec_total_time_ns += dec_kernel_time_ns;

            std::cout << "Round " << round << " sampled token IDs:";
            for (int i = 0; i < FLAGS_gamma; i++) {
                std::cout << " " << sampled_token_idx_mmap[i];
            }
            std::cout << "\n";

            int accepted = FLAGS_accepted_per_round < 0 ? FLAGS_gamma : std::min(std::max(FLAGS_accepted_per_round, 0), FLAGS_gamma);
            int next_last_token_id = 0;
            int commit_count = 0;
            if (accepted < FLAGS_gamma) {
                for (int i = 0; i < accepted; i++) {
                    ids_for_decode.push_back(static_cast<llama_token>(sampled_token_idx_mmap[i]));
                }
                ids_for_decode.push_back(static_cast<llama_token>(FLAGS_replacement_token_id));
                next_last_token_id = FLAGS_replacement_token_id;
                commit_count = accepted + 1;
            } else {
                for (int i = 0; i < FLAGS_gamma; i++) {
                    ids_for_decode.push_back(static_cast<llama_token>(sampled_token_idx_mmap[i]));
                }
                next_last_token_id = sampled_token_idx_mmap[FLAGS_gamma - 1];
                commit_count = FLAGS_gamma;
            }

            int old_seq_len = seq_len;
            for (int offset = 0; offset < commit_count; offset++) {
                commit_device_position_to_canonical(seq_len + offset);
            }
            seq_len += commit_count;
            last_token_id = next_last_token_id;

            std::cout << "Round " << round
                      << " done: kernel_time=" << dec_t_s
                      << "s old_seq_len=" << old_seq_len
                      << " new_seq_len=" << seq_len
                      << " accepted=" << accepted
                      << " commit_count=" << commit_count
                      << " next_last_token=" << last_token_id << "\n";
        }

        char fname[256];
        std::snprintf(fname, sizeof(fname), "my_incremental_sampled_token_idx_%d.txt", run);
        std::ofstream fout(fname);
        if (!fout) {
            std::cerr << "Failed to open " << fname << " for writing\n";
        } else {
            for (int i = 0; i < ids_for_decode.size(); ++i) {
                fout << ids_for_decode[i];
                if (i + 1 < ids_for_decode.size()) fout << " ";
            }
            fout << "\n";
        }

        // 5. decode emitted tokens
        ...
        std::cout << "----------------------------------------\n";
        std::cout << decoded << "\n";
        std::cout << "(incremental decode emitted " << ids_for_decode.size() << " tokens)\n";
        std::cout << "----------------------------------------\n";
        ...
        double dec_t_s = request_dec_total_time_ns * 1e-9;
        std::cout << "U280 FPGA Run " << run << " — incremental decode kernel time total: " << dec_t_s << " s\n";
        std::cout << "U280 FPGA Run " << run << " — emitted sequence length: " << ids_for_decode.size() << " \n";
        if (dec_t_s > 0.0) {
            std::cout << "U280 FPGA Run " << run << " — incremental decode throughput: " << ids_for_decode.size()/dec_t_s << " tokens/s\n";
        }
    }
