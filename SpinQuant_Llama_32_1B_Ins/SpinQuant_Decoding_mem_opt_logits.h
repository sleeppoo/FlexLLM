...
struct sampling_idx_logits_pair{
    int idx;
    float logits;
};
...
void dec_Top_K_Sampling_Embedding_Layer(
    ...
    tapa::mmap<sampling_idx_logits_pair> sampled_token_idx_logits_mmap,
    tapa::ostream<hls::vector<float, T_BLOCK_PARALLEL>>& new_embedding_stream,
    int dec_seq_len
){
    decoder_seq_loop: for (int dec_seq_id = 0; dec_seq_id < dec_seq_len; dec_seq_id++){
        int sampled_idx;
        hls::vector<int, LOGITS_MAX_K> top_k_idx;
        hls::vector<float, LOGITS_MAX_K> top_k_logits;
        dec_Sampling_Embedding_Layer_with_top_k_logits<float, T_QKVO_FFN_BLOCK_PARALLEL, VOCAB_SIZE_PAD, LOGITS_MAX_K, T_BLOCK_PARALLEL, HIDDEN_DIM, T_QKVO_FFN_BLOCK_PARALLEL * DEC_QKVO_FFN_W_PARALLEL>(
            output_stream_vocab_logits, vocab_lib, new_embedding_stream, rand_seeds_mmap[dec_seq_id], sampled_idx, top_k_idx, top_k_logits
        );
        for(int i = 0; i < LOGITS_MAX_K; i++){
            sampled_token_idx_logits_mmap[dec_seq_id * (LOGITS_MAX_K+1) + i] = {top_k_idx[i], top_k_logits[i]};
        }
        sampled_token_idx_logits_mmap[dec_seq_id * (LOGITS_MAX_K+1) + LOGITS_MAX_K] = {sampled_idx, 0.0f};
...
void SpinQuant_Decoding(
    ...
    .invoke(dec_Top_K_Sampling_Embedding_Layer, output_vocab_logits_stream, rand_seeds_mmap, vocab_lib, sampled_token_idx_logits_mmap, new_embedding_stream, dec_seq_len)
